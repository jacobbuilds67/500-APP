...
test('sletning fjerner spillet fra statistikgrundlaget',()=>{const completed={...sample(),id:'done',status:'completed',endedAt:'2026-09-15T12:00:00Z',winnerIds:['a'],rounds:[{points:{a:500,b:300}}]};const other={...sample(),id:'other',status:'completed',endedAt:'2026-09-14T12:00:00Z',winnerIds:['b'],rounds:[{points:{a:200,b:510}}]};const games=[completed,other].filter((game)=>game.id!=='done');assert.equal(games.length,1);assert.deepEqual(winners(games[0]),['b']);assert.equal(totals(games[0]).a,200);});
